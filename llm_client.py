...
class LLMClient:
    def __init__(self, model: Union[genai.GenerativeModel], *, system_instruction: Optional[str] = None):
    ...
    def send_message(self,
        ...
        try:
            if isinstance(self._model, ChatSession):
                # If tools are provided, include them in the message
                return self._model.send_message(message,
            ...
            delay = self._extract_retry_seconds(e)
            if delay is None or delay <= 0:
                delay = 15
            print(f"\nRate limit exceeded. Retrying in {delay} seconds...")
            time.sleep(delay)
            return self.send_message(message, tools=tools, stream=stream, config=config, **generation_kwargs)
